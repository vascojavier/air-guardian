...
import { View, Text, StyleSheet, Dimensions, TouchableOpacity, Image, Image as RNImage, Alert, Platform, AppState, ScrollView } from 'react-native';
//                                             ^^^^^^^^^^^^^^  agrega esto
...
useEffect(() => {
    ...
    // Guard: si ya estoy en tierra/taxi, no calcules TA/RA y limpia UI
  {
    ...
    if (myOps && GROUND_OPS.has(myOps)) {
      ...
      setPlanes(prev => prev.map(p => ({ ...p, alertLevel: 'none', timeToImpact: undefined })));
      return;
    }
  }
  ...
  for (const plane of trafficWithoutMe) {
    ...
    // TA: tráfico cercano (2000 m) solo si ambos están “en vuelo”
    {
      const myAgl = getAGLmeters();
      const otherOps = getOpsOf(plane.id);
      const otherIsGround = otherOps && GROUND_OPS.has(otherOps);

      // Si no tenemos ops del otro, inferimos su AGL aproximado
      const fieldElev = (airfield as any)?.elevation ?? (runwayState?.airfield?.elevation ?? 0);
      ...
      if (
        ...
        !otherIsGround &&          // el otro no en tierra/pista
        (otherOps ? true : otherAgl > 50) // si no hay ops, pedimos AGL>50m
      ) {
...
s.on('conflicto', (data: any) => {
  ...
  // Si YO estoy en tierra/pista, ignorar conflictos entrantes
    if (iAmGroundish()) {
      return;
    }


    // ⛔️ Si el otro ya está en RUNWAY_CLEAR / TAXI_APRON / APRON_STOP, ignorá el conflicto
  const otherName = String(data?.id || data?.name || '');
  if (otherName) {
    const otherOps = getOpsOf(otherName);
    if (otherOps && GROUND_OPS.has(otherOps)) {
      // si justo hay algo mostrado de ese id, lo limpiamos
      clearWarningFor(otherName);
      setBackendWarning(prev => (prev && prev.id === otherName) ? null : prev);
      setPrioritizedWarning(prev => (prev && prev.id === otherName) ? null : prev);
      return; // 👈 no procesamos este conflicto
    }
  }
...
const withinCone = (diffNow <= RA_CONE_DEG) || (diffAtMin <= RA_CONE_DEG);
      ...
      {
        const ops = lastOpsStateRef.current as OpsState | null;
        if (ops === 'RUNWAY_CLEAR' || ops === 'TAXI_APRON' || ops === 'APRON_STOP') {
          if (prioritizedWarning) setPrioritizedWarning(null);
          if (conflict) setConflict(null);
          if (selected) setSelected(null);
          return;
...
<Marker
  ...
  anchor={{ x: 0.5, y: 0.5 }}   // centro del contenedor
  flat
...
>
  <View
    ...
    collapsable={false}      // 👈 importante para que respete el tamaño real
  >
    {/* PNG DEL AVIÓN, CENTRADO Y ROTADO */}
    <RNImage
      source={getOwnPlaneIcon(aircraftIcon)}
      style={{
        width: ICON_SIZE * 0.8,   // 32
        height: ICON_SIZE * 0.8,  // 32
        transform: [{ rotate: `${myPlane.heading}deg` }],
        backfaceVisibility: 'hidden',
      }}
      resizeMode="contain"
    />

    {/* PUNTO CENTRAL (AMARILLO/MAGENTA) */}
    <View
      style={{
        position: 'absolute',
        width: 8,
        height: 8,
        borderRadius: 4,
        backgroundColor: 'yellow',  // o 'magenta'
      }}
    />
  </View>
</Marker>
        ...
        {planes
          ...
          .map((plane) => {
            ...
            return (
              ...
              >
        {/* CONTENEDOR CENTRADO PARA CADA AVIÓN REMOTO (MISMO TAMAÑO QUE MI AVIÓN) */}
        <View
          style={{
            width: 70,   // igual que tu avión propio
            height: 70,
            alignItems: 'center',
            justifyContent: 'center',
          }}
        >
          <RNImage
            ...
            style={{
              width: 56,   // igual que tu avión propio
              height: 56,
              transform: [{ rotate: `${plane.heading || 0}deg` }],
              backfaceVisibility: 'hidden',
            }}
            resizeMode="contain"
          />
